...
const FormProduct = () => {
    ...
    const [mutateSendProducts] = useMutation(API.sendProducts, {refetchQueries: ['products']});
    const [mutateSendMeal] = useMutation(API.sendMeal, { refetchQueries: ['meals']});
    const [mutateRemoveProduct] = useMutation(API.removeProduct, {refetchQueries: ['products']});
    ...
    return (
        ...
        history={history}
        />}
        {location.pathname === '/add-new' && <AddProduct
        onSubmit={onSubmit}
        product={newProduct}
